...
$CONFIG = '{"lang":"fr","error_reporting":false,"show_hidden":false,"hide_Cols":false,"calc_folder":false}';
...
$auth_users = array(
    // 'admin' => '$2y$10$KwjY/CQxCC2YGBB2rBLB3OpEAxrd1EtS244Fd0IevEz5ysZ9jZSD6', //admin@123
...
if ($use_auth) {
    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
        ...
        //Get user ADMIN or SUPER_ADMIN from mon club
        $ch = curl_init($api_url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['username' => $_POST['fm_usr'], 'plainPassword' => $_POST['fm_pwd']]));
        curl_exec($ch);
        ...
        if ($http_code !== 200) {                
            unset($_SESSION[FM_SESSION_ID]['logged']);
            fm_set_msg('Erreur d\'authentification', 'error');
            fm_redirect(FM_SELF_URL);
        } else {            
            $_SESSION[FM_SESSION_ID]['logged'] = $_POST['fm_usr'];
            fm_set_msg('Vous êtes connecté');
            fm_redirect(FM_SELF_URL . '?p=');
        }      
    } else {
        // Form
...
if (isset($_GET['view'])) {
    ...
    $file_url = FM_ROOT_URL . "/file.php?file=". fm_convert_win((FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $file);
    $file_path = $path . '/' . $file;
    ...
    <div class="row">
        <div class="col-12">
            ...
            if($is_onlineViewer) {
                if($online_viewer == 'default') {
                    echo '<embed src="'.fm_enc($file_url).'" style="width:100%;min-height:460px" type="application/pdf">';
                } else if($online_viewer == 'google') {
...
if (isset($_GET['edit'])) {
    ...
    // $file_url = FM_ROOT_URL . fm_convert_win((FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $file);
    $file_url = FM_ROOT_URL . "/file.php?file=". $file;
    $file_path = $path . '/' . $file;
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-white mb-4 main-nav <?php echo $isStickyNavBar ?>">
        <a class="navbar-brand" href=""> <img src="logo.png" style="height: 35px;position: absolute;top: 5px;left: 10px;"/>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo lng('AppTitle') ?> </a>
